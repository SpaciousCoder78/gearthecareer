import mermaid from 'mermaid';
mermaid.initialize({ startOnLoad: true });



function Neuroflow(){
    return(
        <div>
            <head>
                <title>Neurologist Path</title>
            </head>
            <h1>Neurologist Path</h1>
    <pre className="mermaid">
        <img src="https://media.discordapp.net/attachments/1198163202937200682/1198484847790932058/image.png?ex=65bf12fe&is=65ac9dfe&hm=2142ffb41e1df802f252a6d8bb935514638e008213ba70fc976869296c42a3a3&=&format=webp&quality=lossless&width=451&height=739"></img>
</pre>
<p style={{ fontWeight: 'bold' }}> Key Skills needed for a Career in Neurologist are: </p>
    <ol> 
      <li>Dexterity</li>
      <li>Diagnostic skills</li>
      <li>Problem-solving</li>
      <li>Research</li>
    </ol>
<p style={{ fontWeight: 'bold' }}> Career Opportunities in Neurologist are: </p>
<ol>
  <li>Neuro-physician</li>
  <li>Neurosurgeon</li>
  <li>Neurologic Physical Therapist</li>
  <li>Neuro Psychologist and many more..</li>
</ol>
<span className="rating">
  <span className="star"><p style={{ fontWeight: 'bold' }}>Gear the Career's Recommendation: 4.5★</p></span>
</span>
        </div>
    );
}

export default Neuroflow